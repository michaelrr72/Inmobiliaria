 ...
 * @method     ChildUsuariosQuery orderByIdPersona($order = Criteria::ASC) Order by the id_persona column
 *
 ...
 * @method     ChildUsuariosQuery groupByIdPersona() Group by the id_persona column
 *
 ...
 * @method     ChildUsuarios findOneByIdPersona(int $id_persona) Return the first ChildUsuarios filtered by the id_persona column *
 ...
 * @method     ChildUsuarios requireOneByIdPersona(int $id_persona) Return the first ChildUsuarios filtered by the id_persona column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildUsuarios[]|ObjectCollection findByIdPersona(int $id_persona) Return ChildUsuarios objects filtered by the id_persona column
...
abstract class UsuariosQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, login, password, administrador, id_persona FROM usuarios WHERE id = :p0';
        try {
    ...
    public function filterByIdPersona($idPersona = null, $comparison = null)
    {
        if (is_array($idPersona)) {
            ...
            if (isset($idPersona['min'])) {
                $this->addUsingAlias(UsuariosTableMap::COL_ID_PERSONA, $idPersona['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($idPersona['max'])) {
                $this->addUsingAlias(UsuariosTableMap::COL_ID_PERSONA, $idPersona['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(UsuariosTableMap::COL_ID_PERSONA, $idPersona, $comparison);
    }

    /**
     * Filter the query by a related \Personas object
     *
     * @param \Personas|ObjectCollection $personas The related object(s) to use as filter
     * @param string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @throws \Propel\Runtime\Exception\PropelException
     *
    ...
    public function filterByPersonas($personas, $comparison = null)
    {
        if ($personas instanceof \Personas) {
            return $this
                ->addUsingAlias(UsuariosTableMap::COL_ID_PERSONA, $personas->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(UsuariosTableMap::COL_ID_PERSONA, $personas->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
